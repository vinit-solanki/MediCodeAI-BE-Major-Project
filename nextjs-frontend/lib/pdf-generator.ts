...
export const generateClaimPDF = (
  ...
  if (results.evaluation && yPosition < pageHeight - 70) {
    addSection("BACKEND EVALUATION");
    addText(`Overall Verdict: ${results.evaluation.overall_verdict.toUpperCase()}`, 9);
    addText(`Overall Score: ${(results.evaluation.overall_score * 100).toFixed(1)}%`, 9);
    addText(`Compliance Risk: ${results.evaluation.compliance_risk}`, 9);
    if (results.evaluation.summary) {
      const summary = results.evaluation.summary.substring(0, 80);
      addText(`Summary: ${summary}`, 9);
    }
  }
  ...
  doc.text(
    `Trace ID: ${results.traceId || "N/A"} | This claim was generated by MediCode AI`,
    margin,
    pageHeight - 5
  );
